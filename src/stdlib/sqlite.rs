    ...
    fields.insert(
        ...
        PeelValue::NativeFunction(Arc::new(NativeFunc {
            ...
            handler: Arc::new(move |_| {
                ...
                Box::pin(async move {
                    Err(anyhow!("Internal method _get_conn cannot be called directly from Peel"))
                })
            }),
        })),
    );
            ...
            handler: Arc::new(move |args| {
                ...
                Box::pin(async move {
                    let sql = match args.get(0) {
                        ...
                        _ => return Err(anyhow!("Connection.execute expects a SQL string as first argument")),
                    };

                    let params_val = args.get(1).cloned().unwrap_or(PeelValue::List(Arc::new(Mutex::new(vec![]))));
                    let rusql_params = peel_to_rusqlite_params(params_val)?;

                    let conn_lock = conn.lock().unwrap();
                    let c = conn_lock.as_ref().ok_or_else(|| anyhow!("Connection is closed"))?;
                    
                    let rows = c.execute(sql, params_from_iter(rusql_params)).map_err(|e| anyhow!(e))?;
        ...
        PeelValue::NativeFunction(Arc::new(NativeFunc {
            ...
            handler: Arc::new(move |args| {
                ...
                Box::pin(async move {
                    let sql = match args.get(0) {
                        Some(PeelValue::String(s)) => s,
                        _ => return Err(anyhow!("Connection.query expects a SQL string as first argument")),
                    };
                    ...
                    let rows = stmt.query_map(params_from_iter(rusql_params), |row| {
                        ...
                        for (i, name) in names.iter().enumerate() {
                            let val: PeelValue = match row.get_ref(i)? {
                                ...
                                rusqlite::types::ValueRef::Text(t) => PeelValue::String(String::from_utf8_lossy(t).into_owned()),
                                rusqlite::types::ValueRef::Blob(b) => PeelValue::String(format!("<blob {} bytes>", b.len())),
                            };
...
fn peel_to_rusqlite_params(val: PeelValue) -> anyhow::Result<Vec<rusqlite::types::Value>> {
    match val {
        PeelValue::List(l) => {
            let params: Vec<rusqlite::types::Value> = l.lock().unwrap().iter().map(|item| {
                match item {
