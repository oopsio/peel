...
static void KeyExpansion(uint8_t* RoundKey, const uint8_t* Key) {
  ...
  for (i = Nk; i < Nb * (Nr + 1); ++i) {
    ...
    if (i % Nk == 4) { tempa[0] = sbox[tempa[0]]; tempa[1] = sbox[tempa[1]]; tempa[2] = sbox[tempa[2]]; tempa[3] = sbox[tempa[3]]; }
    j = i * 4; k = (i - Nk) * 4;
...
static void AddRoundKey(uint8_t round, uint8_t (*state)[4], const uint8_t* RoundKey) {
  uint8_t i, j;
  for (i = 0; i < 4; ++i) { for (j = 0; j < 4; ++j) { state[i][j] ^= RoundKey[(round * Nb * 4) + (i * Nb) + j]; } }
}
...
static void MixColumns(uint8_t (*state)[4]) {
  uint8_t i, Tmp, Tm, t;
  for (i = 0; i < 4; ++i) {
    t = state[i][0]; Tmp = state[i][0] ^ state[i][1] ^ state[i][2] ^ state[i][3];
...
void AES_CBC_encrypt_buffer(struct AES_ctx* ctx, uint8_t* buf, size_t length) {
  ...
  for (i = 0; i < length; i += AES_BLOCKLEN) {
    size_t j; for (j = 0; j < AES_BLOCKLEN; ++j) { buf[i + j] ^= Iv[j]; }
...
static void InvMixColumns(uint8_t (*state)[4]) {
  ...
  for (i = 0; i < 4; ++i) {
    ...
    state[i][3] = Multiply(a, 0x0b) ^ Multiply(c, 0x09) ^ Multiply(d, 0x0e) ^ Multiply(b, 0x0d);
  }
}
...
void AES_CBC_decrypt_buffer(struct AES_ctx* ctx, uint8_t* buf, size_t length) {
  ...
  for (i = 0; i < length; i += AES_BLOCKLEN) {
    ...
    size_t j; for (j = 0; j < AES_BLOCKLEN; ++j) { buf[i + j] ^= ctx->Iv[j]; }
