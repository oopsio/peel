...
pub fn register() -> HashMap<String, PeelValue> {
    ...
    methods.insert("parse".to_string(), PeelValue::NativeFunction(Arc::new(NativeFunc {
    ...
    methods.insert("stringify".to_string(), PeelValue::NativeFunction(Arc::new(NativeFunc {
...
fn json_to_peel(json: JsonValue) -> PeelValue {
    match json {
        ...
        JsonValue::Number(n) => {
            if let Some(i) = n.as_i64() { PeelValue::Int(i) }
            else { PeelValue::Float(n.as_f64().unwrap_or(0.0)) }
        }
        JsonValue::String(s) => PeelValue::String(s),
        JsonValue::Array(arr) => PeelValue::List(Arc::new(Mutex::new(arr.into_iter().map(json_to_peel).collect()))),
        JsonValue::Object(obj) => {
            let mut fields = HashMap::new();
            for (k, v) in obj {
                fields.insert(k, json_to_peel(v));
            }
            PeelValue::Object { struct_name: None, fields: Arc::new(Mutex::new(fields)) }
        }
    }
}
...
fn peel_to_json(peel: &PeelValue) -> JsonValue {
    match peel {
        ...
        PeelValue::Float(f) => JsonNumber::from_f64(*f).map(JsonValue::Number).unwrap_or(JsonValue::Null),
