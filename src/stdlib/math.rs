...
pub fn register() -> HashMap<String, PeelValue> {
    ...
    methods.insert("floor".to_string(), create_math_fn_1("floor", |x| x.floor()));
    methods.insert("ceil".to_string(), create_math_fn_1("ceil", |x| x.ceil()));
    methods.insert("round".to_string(), create_math_fn_1("round", |x| x.round()));
    methods.insert("log".to_string(), create_math_fn_1("log", |x| x.ln()));

    methods.insert("pow".to_string(), PeelValue::NativeFunction(Arc::new(NativeFunc {
        name: "pow".to_string(),
    ...
    methods.insert("random".to_string(), PeelValue::NativeFunction(Arc::new(NativeFunc {
