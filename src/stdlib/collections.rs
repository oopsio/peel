...
pub fn register_string() -> HashMap<String, PeelValue> {
    ...
    methods.insert("toUpperCase".to_string(), PeelValue::NativeFunction(Arc::new(NativeFunc {
    ...
    methods.insert("len".to_string(), PeelValue::NativeFunction(Arc::new(NativeFunc {
        name: "len".to_string(),
        handler: Arc::new(|args| Box::pin(async move {
            if let Some(PeelValue::String(s)) = args.get(0) {
...
pub fn register_array() -> HashMap<String, PeelValue> {
    ...
    methods.insert("push".to_string(), PeelValue::NativeFunction(Arc::new(NativeFunc {
        name: "push".to_string(),
        handler: Arc::new(|args| Box::pin(async move {
    ...
    methods.insert("pop".to_string(), PeelValue::NativeFunction(Arc::new(NativeFunc {
        name: "pop".to_string(),
        handler: Arc::new(|args| Box::pin(async move {
            if let Some(PeelValue::List(l)) = args.get(0) {
